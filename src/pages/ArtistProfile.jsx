    ...
    reviews: [
      {
        user: "娃娃控",
        rating: 5,
        content: "妆面超级精致！细节处理太棒了",
        date: "2024-12-15",
      },
      {
        user: "古风爱好者",
        rating: 5,
        content: "完美还原了我想要的感觉，下次还来",
        date: "2024-12-10",
      },
    ],
  },
...
const getArtistById = (id) =>
  MOCK_ARTISTS.find((a) => a.id === id) || MOCK_ARTISTS[0];
...
const ArtistProfile = () => {
  ...
  const [chatHistory, setChatHistory] = useState([
    { type: "sys", text: "已为您连接大师，请直接留言。" },
  ]);

  const handleSendChat = () => {
    if (!chatMsg.trim()) return;
    setChatHistory((prev) => [...prev, { type: "me", text: chatMsg }]);
    setChatMsg("");
    setTimeout(() => {
      setChatHistory((prev) => [
        ...prev,
        {
          type: "other",
          text: "您好，目前排单较满，您可以先看下服务详情，有疑问随时留言~",
        },
      ]);
    }, 1000);
  };
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-white px-4 py-5">
        <div className="flex items-start gap-4">
          <div
            className={`w-20 h-20 rounded-2xl ${artist.avatar} flex-shrink-0`}
          />
        ...
        <button
          ...
          className={`w-full mt-4 py-2.5 rounded-full text-sm font-medium transition-all ${
            isFollowing ? "bg-gray-100 text-gray-600" : "bg-rose-500 text-white"
          }`}
        >
          {isFollowing ? "已关注" : "+ 关注"}
        </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 py-3 flex items-center gap-3">
        <button
          ...
          className="flex-1 py-3 bg-gray-100 text-gray-700 rounded-full text-sm font-medium flex items-center justify-center gap-2"
        >
        ...
        <button
          ...
          className="flex-1 py-3 bg-rose-500 text-white rounded-full text-sm font-medium flex items-center justify-center gap-2"
        >
          <Calendar size={18} />
          预约服务
        </button>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50">
          <div className="bg-white w-full rounded-t-2xl p-4 animate-in slide-in-from-bottom duration-300">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-lg">预约服务</h3>
              <button
            ...
            <div className="space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                ...
                <div className="space-y-2">
                  {artist.services.map((service, idx) => (
                    <div
                      key={idx}
                      className="border border-gray-200 rounded-xl p-3 flex justify-between items-center active:border-rose-500 active:bg-rose-50 transition-colors cursor-pointer"
                    >
                      <div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button className="flex-1 py-2 border border-rose-500 bg-rose-50 text-rose-600 rounded-lg text-sm font-medium">
                    近期 (1个月内)
                  </button>
                  <button className="flex-1 py-2 border border-gray-200 text-gray-600 rounded-lg text-sm">
                    远期 (3个月后)
                  </button>
                </div>
              </div>
              <div className="pt-2">
                <button
                  onClick={() => {
                    alert("预约申请已发送，等待大师确认");
                    setShowAppointment(false);
                  }}
                  className="w-full bg-rose-500 text-white py-3 rounded-xl font-bold text-sm shadow-lg shadow-rose-200 active:scale-98 transition-transform"
                >
                  确认预约
                </button>
              </div>
      ...
      {showChat && (
        <div className="fixed inset-0 z-50 bg-gray-50 flex flex-col animate-in slide-in-from-right duration-300">
          <div className="bg-white px-4 py-3 flex items-center gap-3 border-b border-gray-100">
            ...
            <div className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full ${artist.avatar}`} />
          ...
          <div className="bg-white p-3 border-t border-gray-100 pb-6">
            <div className="bg-gray-100 rounded-full flex items-center px-4 py-2 gap-2">
              ...
              <button
                ...
                className={`p-1.5 rounded-full transition-colors ${chatMsg.trim() ? "bg-rose-500 text-white" : "text-gray-400"}`}
              >
                <Send size={16} />
              </button>
            </div>
