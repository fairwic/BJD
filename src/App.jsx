...
const ProductCard = ({ item, onClick }) => (
  <div onClick={onClick} className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 group active:scale-95 transition-transform">
    ...
    <div className="p-3">
      <h3 className="font-medium text-gray-800 text-sm line-clamp-2 h-10">{item.title}</h3>
...
// 筛选栏组件
const FilterBar = ({ activeCategory, setActiveCategory, activeSize, setActiveSize }) => {

  return (
    <div className="bg-white sticky top-[57px] z-20 shadow-sm">
      {/* 尺寸筛选 */}
      <div className="flex overflow-x-auto px-4 py-2 gap-2 scrollbar-hide border-b border-gray-50">
        {CATEGORY_CONFIG.SIZES.map(size => (
          <button
            key={size.id}
            onClick={() => setActiveSize(size.id)}
            className={`whitespace-nowrap px-3 py-1 rounded-full text-xs transition-colors ${activeSize === size.id
      ...
      {/* 类型筛选 */}
      <div className="flex overflow-x-auto px-4 py-3 gap-4 scrollbar-hide">
        {CATEGORY_CONFIG.TYPES.map(type => (
          <button
            key={type.id}
            onClick={() => setActiveCategory(type.id)}
            className={`whitespace-nowrap text-sm font-medium transition-colors relative ${activeCategory === type.id ? 'text-rose-500' : 'text-gray-500'
              }`}
          >
            {type.label}
            {activeCategory === type.id && (
              <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 w-4 h-1 bg-rose-500 rounded-full" />
            )}
          </button>
        ))}
      </div>
...
const UserShop = () => {
  ...
  const [activeBrandStyle, setActiveBrandStyle] = useState('all'); // 'all', 'japanese', 'chinese'

  // 筛选逻辑
  const filteredProducts = MOCK_PRODUCTS.filter(p => {
    const matchCategory = activeCategory === 'all' || p.category === activeCategory;
    const matchSize = activeSize === 'all' || p.size === 'all' || p.size === activeSize;
  ...
  return (
    <div className="pb-20 h-full flex flex-col">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-gray-100">
        <div className="flex px-4 pt-4 pb-2 gap-6 items-end">
          <button
            onClick={() => setActiveSubTab('merchant')}
            className={`text-lg font-bold transition-colors ${activeSubTab === 'merchant' ? 'text-gray-900' : 'text-gray-400'
              }`}
          >
            现货市集
          </button>
          <button
            ...
            className={`text-lg font-bold transition-colors ${activeSubTab === 'leader' ? 'text-gray-900' : 'text-gray-400'
              }`}
          >
            开团广场
          </button>
          <div className="ml-auto flex gap-3 text-gray-600">
            <Search size={20} />
            <ShoppingBag size={20} onClick={() => push('MerchantProfile', { name: '爱丽丝的衣橱' })} />
          </div>
        </div>
        {/* 指示条 */}
        <div className="relative px-4">
          <div className={`absolute bottom-0 h-0.5 bg-rose-500 w-8 transition-all duration-300 ${activeSubTab === 'merchant' ? 'left-8' : 'left-[130px]'
            }`} />
        </div>
      </div>
      ...
      {activeSubTab === 'merchant' && (
        <div className="bg-white border-b border-gray-100 px-4 py-3">
          ...
          {/* 品牌快捷选择 */}
          {activeBrandStyle !== 'all' && (
            <div className="mt-2 flex gap-2 overflow-x-auto scrollbar-hide">
              {activeBrandStyle === 'japanese' ? (
                <>
                  <span className="px-2 py-1 bg-blue-50 text-blue-600 text-[10px] rounded-full whitespace-nowrap">VOLKS</span>
                  <span className="px-2 py-1 bg-blue-50 text-blue-600 text-[10px] rounded-full whitespace-nowrap">Fairyland</span>
                  <span className="px-2 py-1 bg-blue-50 text-blue-600 text-[10px] rounded-full whitespace-nowrap">Luts</span>
                  <span className="px-2 py-1 bg-blue-50 text-blue-600 text-[10px] rounded-full whitespace-nowrap">Iplehouse</span>
                </>
              ) : (
                <>
                  <span className="px-2 py-1 bg-red-50 text-red-600 text-[10px] rounded-full whitespace-nowrap">Ringdoll</span>
                  <span className="px-2 py-1 bg-red-50 text-red-600 text-[10px] rounded-full whitespace-nowrap">DollZone</span>
                  <span className="px-2 py-1 bg-red-50 text-red-600 text-[10px] rounded-full whitespace-nowrap">Doll Chateau</span>
                  <span className="px-2 py-1 bg-red-50 text-red-600 text-[10px] rounded-full whitespace-nowrap">XAGA</span>
                  <span className="px-2 py-1 bg-red-50 text-red-600 text-[10px] rounded-full whitespace-nowrap">AS</span>
                  <span className="px-2 py-1 bg-red-50 text-red-600 text-[10px] rounded-full whitespace-nowrap">GEM</span>
                </>
              )}
            </div>
          )}
      ...
      <div className="p-4 flex-1 overflow-y-auto">
        {activeSubTab === 'merchant' ? (
          ...
          <div className="space-y-4">
            {/* 团长发布的图文卡片 */}
            {groupBuys.map(g => (
              <div key={g.id} onClick={() => push('ProductDetail', { id: g.id })} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden active:scale-95 transition-transform">
                <div className={`h-40 w-full ${g.image} relative`}>
                ...
                <div className="p-3">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-xs text-gray-500">{g.leader}</span>
                  </div>
                  <div className="mt-3 flex items-center justify-between border-t border-gray-50 pt-2">
                    <div className="flex gap-4 text-gray-500 text-xs">
                      <span className="flex items-center gap-1"><Heart size={14} /> 102</span>
                      <span className="flex items-center gap-1"><MessageSquare size={14} /> 45</span>
                    </div>
                    <button className="bg-rose-500 text-white text-xs px-3 py-1 rounded-full">去参团</button>
                  </div>
                </div>
              </div>
            ))}
